...
async function saveQuiz(){ try{ if(isEdit.value){ await axios.put(`/admin/quizzes/${quizForm.value.id}`, quizForm.value) }else{ await axios.post(`/admin/chapters/${quizForm.value.chapter_id}/quizzes`, quizForm.value) } }catch(e){console.error(e)} close() }
